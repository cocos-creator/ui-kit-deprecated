...
export default class ScrollViewComponent extends UIElement {
  constructor() {
    ...
    this._vertical = true;
    this._movementType = 'unrestricted';
    this._elastic = 0.1;
  ...
  _getBound() {
    ...
    let viewRect = this._viewWidget._rect;
    ...
    lcorner[1] = vec3.new(lpos.x + this._contentWidget._rect.x, lpos.y + this._contentWidget._rect.y, 0);
    lcorner[0] = vec3.new(lcorner[1].x, lcorner[1].y + this._contentWidget._rect.h, 0);
    lcorner[2] = vec3.new(lcorner[1].x + this._contentWidget._rect.w, lcorner[1].y, 0);
    lcorner[3] = vec3.new(lcorner[1].x + this._contentWidget._rect.w, lcorner[1].y + this._contentWidget._rect.h, 0);
  ...
  _setHorizontalOffset(val) {
    if (this._contentWidget.anchorLeft === this._contentWidget.anchorRight) {
      this._contentWidget.offsetX = val;
    } else {
      this._contentWidget.marginRight += this._contentWidget.marginLeft - val;
      this._contentWidget.marginLeft = val;
    }
  }

  _setVerticalOffset(val) {
    if (this._contentWidget.anchorBottom === this._contentWidget.anchorTop) {
      this._contentWidget.offsetY = val;
    } else {
      this._contentWidget.marginTop += this._contentWidget.marginBottom - val;
      this._contentWidget.marginBottom = val;
    }
  }
  ...
  _getCalculateOffset() {
    let offset = { x: 0, y: 0 };
    if (this._contentWidget.anchorLeft === this._contentWidget.anchorRight) {
      offset.x = this._contentWidget.offsetX;
    } else {
      offset.x = this._contentWidget.marginLeft;
    }

    if (this._contentWidget.anchorBottom === this._contentWidget.anchorTop) {
      offset.y = this._contentWidget.offsetY;
    } else {
      offset.y = this._contentWidget.marginBottom;
    }
...
ScrollViewComponent.schema = {
  ...
  vScrollBar: {
    ...
    set(val) {
      if (this._vScrollBar && this._vScrollBar._entity === val) {
        return;
      }
      ...
      this._vScrollBar = val.getComp('ScrollBar');
      if (this._vScrollBar) {
  ...
  hScrollBar: {
    ...
    set(val) {
      if (this._hScrollBar && this._hScrollBar._entity === val) {
        return;
      }
      ...
      this._hScrollBar = val.getComp('ScrollBar');
      if (this._hScrollBar) {
