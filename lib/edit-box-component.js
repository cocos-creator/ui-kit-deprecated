...
export default class EditBoxComponent extends UIElement {
  ...
  _registerEvent() {
    ...
    this._activeDom.addEventListener('keypress', this._onSubmit);
  }

  _unregisterEvent() {
    console.log('.....................unregister event');
    this._activeDom.removeEventListener('input', this._onInput);
    this._activeDom.removeEventListener('keypress', this._onSubmit);
  }
...
EditBoxComponent.schema = {
  textComp: {
    ...
    default: null,
    set(val) {
      if (this._textComp && this._textComp._entity === val) {
        return;
      }

      if (this._textComp) {
        this._unregisterTextCompEvents();
      }

      this._textComp = val.getComp('Text');
      this._registerTextCompEvents();
      if (!this._textComp) {
  ...
  placeHolder: {
    ...
    default: null,
    set(val) {
      if (this._placeHolder && this._placeHolder._entity === val) {
        return;
      }

      if (this._placeHolder) {
        this._unregisterPlaceHolderEvents();
      }

      this._placeHolder = val.getComp("Text");
