...
export default class Bounds {
  ...
  encapsulate(point) {
    let mi = vec3.min(this.min, this.min, point);
    let ma = vec3.max(this.max, this.max, point);
    this.setMinMax(vec3.min(this.min, this.min, point), vec3.max(this.max, this.max, point));
  }
}
