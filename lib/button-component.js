...
export default class ButtonComponent extends UIElement {
  ...
  onEnable() {
    this._widget.system._screens[0]._entity.on('touchend', this._onTouchScreenEnd );
  }

  onDisable() {
    this._widget.system._screens[0]._entity.off('touchend', this._onTouchScreenEnd );
  }
...
ButtonComponent.schema = {
  ...
  label: {
    ...
    set(val) {
      if (this._labelComp) {
        this._labelComp.text = val;
      }
    },
    get() {
      return this._labelComp.text;
    }
  },
